...
#+end_src
** CLOSEOUT [[file:config/int_test.yaml][Snakemake configuration YAML]]
CLOSED: [2023-01-26 Thu 12:10]
:PROPERTIES:
...
#+end_src
**** Get bams to downsample
def get_ds_bams(wildcards):
    qc_checkp_out = checkpoints.cfdna_wgs_make_qc_tsv.get(**wildcards).output[0]
    return expand(
*** All rule
#+begin_src snakemake
rule all:
...
*** Use
- See config/int_test.yaml for necessary run conditions.
- Getting started
