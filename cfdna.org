...
rule frag_multiqc:
    input:
        expand(f"{qc_dir}/{{library}}_cfdna_wgs_fastp.json", library = FRAG_LIBS),
        expand(f"{qc_dir}/{{library}}_{{processing}}_{{read}}_fastqc.zip",
               library = FRAG_LIBS,
               processing = ["raw", "proc", "unpaired"],
               read = ["R1","R2"]),
        expand(f"{qc_dir}/{{library}}_{{processing}}_samstats.txt",
               library = FRAG_LIBS,
               processing = ["raw","filt"]),
        expand(f"{qc_dir}/{{library}}_{{processing}}_flagstat.txt",
               library = FRAG_LIBS,
               processing = ["raw","filt"]),
        expand(f"{qc_dir}/{{library}}_picard_depth.txt",
               library = FRAG_LIBS),
        f"{qc_dir}/deeptools_frag_lengths.txt",
        f"{qc_dir}/frag_coverage.tsv",
    log: f"{log_dir}/frag_multiqc.log",
    output:
        f"{qc_dir}/frag_multiqc.html",
        f"{qc_dir}/frag_multiqc_data/multiqc_fastqc.txt",
        f"{qc_dir}/frag_multiqc_data/multiqc_samtools_stats.txt",
        f"{qc_dir}/frag_multiqc_data/multiqc_picard_wgsmetrics.txt",
        f"{qc_dir}/frag_multiqc_data/multiqc_samtools_flagstat.txt",
    params:
        out_dir = f"{qc_dir}",
