...
#+begin_src bash
ls /mnt/gcs/ris/mpnst/analysis/cfdna-wgs/bams | grep lib18

for file in /mnt/gcs/ris/mpnst/analysis/cfdna-wgs/bams/lib18*ds10.bam*; do
    rsync -avh $file /mnt/data/projects/pnst/bams/
done
...
#+begin_src bash
mamba env update --name cfdna --file ./basecamp/basecamp_env.yaml
mamba env update --name cfdna --file ./biotools/biotools_env.yaml
#+end_src

** [[https://github.com/jeszyman/cfdna][GitHub]]
...
rule make_ds_targets:
    ...
    run:
        with open(output[0], "w") as f:
            f.write("\n".join(input))
#+end_src
*** cfDNA EM-seq processing and QC
- insert size density plot
-
*** Basic WGS cfDNA read processing and QC [[file:workflow/frag_reads.smk][Basic read processing]]        :smk:
:PROPERTIES:
...
*** End motifs
#+begin_src bash
python3 ~/repos/biotools/scripts/subsample_bam.py --bam_file /mnt/data/projects/pnst/bams/lib180_ds10.bam --target_mb 20 --output_name human1.bam

python3 ~/repos/biotools/scripts/subsample_bam.py --bam_file /mnt/data/projects/pnst/bams/lib180_ds10.bam --target_mb 20 --output_name human2.bam

python3 ~/repos/biotools/scripts/subsample_bam.py --bam_file /mnt/data/projects/pnst/bams/lib180_ds10.bam --target_mb 20 --output_name human3.bam

python3 ~/repos/biotools/scripts/subsample_bam.py --bam_file /mnt/data/projects/pnst/bams/lib180_ds10.bam --target_mb 20 --output_name human4.bam

python3 ~/repos/biotools/scripts/subsample_bam.py --bam_file /mnt/data/projects/pnst/bams/lib180_ds10.bam --target_mb 20 --output_name human5.bam
#+end_src
...
****** Shell
#+begin_src bash
mkdir -p ~/repos/cfdna/data/motifs

# ~/repos/cfdna/scripts/sample_motifs.sh \
#     ~/repos/cfdna/data/bams/human1.bam \
#     ~/pnst/inputs/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna \
#     4 \
#     10000 \
#     .1 \
#     4 \
#     ~/repos/cfdna/data/motifs/human1.txt

#!/bin/bash

BAM_DIR=~/repos/cfdna/data/bams
REF_GENOME=~/pnst/inputs/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna
MOTIF_DIR=~/repos/cfdna/data/motifs
THREADS=4
REGION_SIZE=10000
FRACTION=.1
NUM_REGIONS=4
...
for BAM_FILE in "$BAM_DIR"/*.bam; do
    ...
    OUTPUT_FILE="$MOTIF_DIR/${BASENAME}.txt"
...
- [ ] output is really double the input nreads
- [ ] pythonize?
...
# Create a DataFrame of the F-profiles
F_df = pd.DataFrame(F, columns=motif_names, index=[f'fprof{i+1}' for i in range(n_components)])

# Save the F-profiles to a TSV file
F_df.to_csv('/tmp/fdf.tsv', sep='\t', index=True)
...
#
motifs_df = pd.read_csv('/tmp/motifs.tsv', delimiter='\t', dtype=str)


# Convert the filtered DataFrame to a NumPy array
motifs = motifs_df.to_numpy()

#M = motifs[1:, 1:].astype(float)
M = motifs[:, 1:].astype(float)
M = M.T
...
#########1#########2#########3#########4#########5#########6#########7#########8
# Initialize an empty DataFrame to store the NNLS outputs
nnls_contributions_df = pd.DataFrame()

# Iterate over each library (sample) in the DataFrame
for lib in motifs_df.columns[1:]:  # Exclude the 'motif' column
# Extract the motif frequencies for the current library
    sample_motif_frequencies = motifs_df[lib].values.astype(float)

    # Perform NNLS to get the contributions of each F-profile
    P, _ = nnls(F.T, sample_motif_frequencies)

    # Normalize P so that the sum equals 100%
    P_normalized = 100 * P / np.sum(P)

    # Add the normalized contributions to the DataFrame
    nnls_contributions_df[lib] = P_normalized

# Optionally, if you want to add row labels corresponding to the component names
nnls_contributions_df.index = ['fprof{}'.format(i) for i in range(1, n_components+1)]
...
nnls_contributions_df.to_csv('/tmp/nnls_contributions.tsv', sep='\t')
#########1#########2#########3#########4#########5#########6#########7#########8
motif_names = motifs[:, 0]
...
# Normalize the rows of the F matrix so that each row sums to 1
F_normalized = F / F.sum(axis=1, keepdims=True)

# Convert the normalized matrix to a DataFrame
F_df = pd.DataFrame(F_normalized, columns=motif_names, index=[f'fprof{i+1}' for i in range(n_components)])

# Save the DataFrame to a file
F_df.to_csv('/tmp/fdf_normalized.tsv', sep='\t', index=True)
