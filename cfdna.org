...
:header-args:snakemake: :tangle ./workflows/cfdna_cna.smk
:END:
[[file:workflows/cfdna_cna.smk]]
...
snakefile = Path("./workflows/cfdna_cna.smk").read_text()
...
#+RESULTS[d4e466324f6e5001e7262eb9bcf78df5bbd75062]:
...
#ssh jeff-nf1
#conda activate base
#mamba env create --file ~/repos/cfdna/config/cfdna-cna-conda-env.yaml
#conda activate cfdna_wgs
...
usage() {
    cat <<EOF
Usage: $(basename "$0") <in_bam> <out_wig>

Generate a WIG from a BAM using readCounter.

Positional arguments:
  in_bam   Path to input BAM file
  out_wig  Path to output WIG file
EOF
    exit 1
}
...
#IN_BAM=/mnt/gcs/jeszyman/projects/nf1/cfdna-wgs/bams/kh_02.ncbi_decoy_hg38.bwa.dedup.coorsort.filt.bam

IN_BAM=/mnt/gcs/jeszyman/projects/nf1/cfdna-wgs/bams/kh_02.ncbi_decoy_hg38.bwa.coorsort.bam

IN_BAM=/mnt/gcs/ris/mpnst/analysis/cfdna-wgs/bams/lib424_filt.bam

IN_BAM=/mnt/gcs/ris/mpnst/analysis/cfdna-wgs/cna/frag_bams/lib424_frag90_150.bam

IN_BAM=/mnt/gcs/jeszyman/projects/nf1/emseq/bams/emseq_06.ncbi_decoy_hg38.bwa_meth.coorsort.deduped.bam
OUT_WIG=/mnt/gcs/jeszyman/projects/nf1/tmp/TEST3.wig

# parameters (override via env if desired)
...
#mkdir -p /tmp/ichor
mkdir -p /mnt/gcs/jeszyman/projects/nf1/tmp/ichor

Rscript ~/repos/ichorCNA-patched/scripts/runIchorCNA.R \
        --id TEST3 \
        --WIG $OUT_WIG \
        --normal "c(0.95, 0.99, 0.995, 0.999)" \
        --genomeBuild hg38 \
        --ploidy "c(2)" \
        --gcWig ~/repos/ichorCNA-patched/inst/extdata/gc_hg38_1000kb.wig \
        --mapWig ~/repos/ichorCNA-patched/inst/extdata/map_hg38_1000kb.wig \
        --centromere ~/repos/ichorCNA-patched/inst/extdata/GRCh38.GCA_000001405.2_centromere_acen.txt \
        --normalPanel ~/repos/ichorCNA-patched/inst/extdata/HD_ULP_PoN_1Mb_median_normAutosome_mapScoreFiltered_median.rds \
        --includeHOMD FALSE \
        --chrs "c(1:22)" \
        --chrTrain "c(1:22)" \
        --estimateNormal TRUE \
        ...
        --txnE 0.9999 \
        --txnStrength 10000 \
        --outDir /mnt/gcs/jeszyman/projects/nf1/tmp/ichor \
        --libdir ~/repos/ichorCNA-patched

cat /mnt/gcs/jeszyman/projects/nf1/tmp/ichor/TEST.params.txt
cat /mnt/gcs/jeszyman/projects/nf1/tmp/ichor/TEST2.params.txt
cat /mnt/gcs/jeszyman/projects/nf1/tmp/ichor/TEST3.params.txt
...
#+begin_src bash
# conda activate base
# mamba env create -f ~/repos/cfdna/config/cfdna-cna-conda-env.yaml
...
rule cfdna_cna_extract_tumor_fractions:
    conda:
        f"{config['cfdna-cna-conda-env']}",
    input:
        expand(f"{config['cfdna-cna-dir']}/ichor/{{wkflow_id}}.frag{{frag_distro}}.ds{{mil_reads}}/{{wkflow_id}}.frag{{frag_distro}}.ds{{mil_reads}}.params.txt", wkflow_id = cfdna_cna_wkflow_ids),
    output:
        f"{config['cfdna-cna-dir']}/ichor/ichor_tumor_fractions.tsv"
    run:
...
#+end_src
*** Reference
- Coverage-based CNA from scratch [cite:@vavoulis2025]
*** Ideas
- https://www.medrxiv.org/content/10.1101/19002550v3.full.pdf+html
  - In order to maximize sensitivity of CNAs while preventing false positive detections, Spearman’s rank correlation between local mean fragment length and copy number was used as a disqualifier of CNA call sets (Hellwig et al. 2018; Mouliere et al. 2018
- Build out Landau MRD-EDGECNV (CN denoising, BAF, and fragment-length entropy)
  - https://www.annalsofoncology.org/article/S0923-7534(19)31323-7/fulltext
  - https://github.com/mskilab-org/dryclean https://www.biorxiv.org/content/10.1101/847681v2.full
  - [cite:@widman2024]
** End motifs
